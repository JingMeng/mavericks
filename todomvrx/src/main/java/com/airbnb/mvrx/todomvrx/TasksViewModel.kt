...
class TasksViewModel(initialState: TasksState, private val sources: List<TasksDataSource>) :
    ...
    fun setComplete(id: String, complete: Boolean) {
        setState {
            ...
            copy(
                tasks = tasks.copy(tasks.indexOf(task), task.copy(complete = complete)),
                lastEditedTask = id
            )
        }
    ...
    fun clearCompletedTasks() = withState {

        setState {
            Log.i("TaskListFragment789", "-----222222------${it.count}-------------")
            copy(count = count + 1)
        }
        ...
        viewModelScope.launch {
            withContext(Dispatchers.IO){
                repeat(10){
                    delay(1000)
                    setState {
//                        Log.i("TaskListFragment789", "-----333333333------${it.count}-------------")
                        copy(count = count + 1)
                    }
                }
